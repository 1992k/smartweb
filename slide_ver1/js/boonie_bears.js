$(function(){
	...
	/*
		...
		2. 그  index번호에 해당하는 a요소의 부모에, .on클래스를 추가한다
		3. 그 index번호에 해당하는 a요소의 부모의 형제에 해당하는 다른 li에게서 .on클래스를 제거한다.
		4. index번호에 매칭되는 슬라이드가 보일수 있도록 .slides-container를 이동한다.

	*/

	// 제잌쿼리로 셀렉팅할 경우 배열 방식으로 저장된다.
	...
	$indicator.click(function(){
		...
		// .index() 메소드는 지정된 요소의 인덱스번호를 추출한다 ( 0 부터 시작 ) 
